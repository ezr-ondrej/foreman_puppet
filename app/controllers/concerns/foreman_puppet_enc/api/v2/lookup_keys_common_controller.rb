module ForemanPuppetEnc::Api::V2::LookupKeysCommonController
  ...
  def find_smart_class_parameter
    ...
    @smart_class_parameter = ForemanPuppetEnc::PuppetclassLookupKey.authorized(:view_external_parameters).smart_class_parameters.find_by_id(id.to_i) if id.to_i > 0
    @smart_class_parameter ||= begin
                                 ...
                                 ForemanPuppetEnc::PuppetclassLookupKey.authorized(:view_external_parameters).smart_class_parameters.where(puppet_cond).where(env_cond).where(:key => id).first
                               end
  ...
  def smart_class_parameters_resource_scope
    base = ForemanPuppetEnc::PuppetclassLookupKey.authorized(:view_external_parameters)
